...
class SubprojectCommonComponent(SubprojectMixin, LoginRequiredMixin, generic.ListView):
    ...
    def get_results(self):
        subprojects = Subproject.objects.get_objects_by_general_filtre(self.request, None).filter(full_title_of_approved_subproject=self.subproject.full_title_of_approved_subproject).get_actifs()
        return {
...
class SubprojectsByAdministrativeLevelMixin(AdministrativeLevelMixin):
    ...
    def dispatch(self, request, pk: int, *args, **kwargs):
        ...
        if self.administrative_level.type == "Canton":
            ...
            self.subprojects = list(Subproject.objects.get_objects_by_general_filtre(self.request, None).filter(canton_id=self.administrative_level.id).get_actifs())
        ...
        for v in self.villages:
            self.subprojects += list(Subproject.objects.get_objects_by_general_filtre(self.request, None).filter(location_subproject_realized_id=v.id).get_actifs())
