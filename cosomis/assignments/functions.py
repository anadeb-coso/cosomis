...
def get_stabilized_administrativelevels_of_facilitators_by_project_id(facilitator, project_id, type_adl="Village", parent_id=None) -> _QS:
    ...
    # Ancien code : `nsc.get_db('eadls').get_query_result({"type": "adl", "representative.email": facilitator.email})`.
    # La base CouchDB `eadls` a été migrée vers Postgres côté GRM (`issue.models.Adl`) : on
    # interroge désormais l'API inter-services GRM par email. Le dict renvoyé porte les mêmes
    # clés `administrative_regions`/`additional_administrative_regions` que l'ancien document
    # CouchDB, ce qui rend ce remplacement quasiment "drop-in".
    all_administrative_ids = (facilitator.stabilization_administrative_ids or []) + (facilitator.additional_administrative_ids or [])

    if not all_administrative_ids:
        facilitator_stabilized = grm_client.get_facilitator_by_email(facilitator.email)

        administrativelevels = []
        all_administrative_ids = []
        if facilitator_stabilized:
            all_administrative_ids = facilitator_stabilized['administrative_regions']
            if 'additional_administrative_regions' in facilitator_stabilized and facilitator_stabilized['additional_administrative_regions']:
                if not all_administrative_ids:
                    all_administrative_ids = []
                all_administrative_ids = all_administrative_ids + facilitator_stabilized['additional_administrative_regions']
